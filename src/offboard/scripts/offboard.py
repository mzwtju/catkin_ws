...
class Leader:
    def __init__(self):
        ...
        self.motion_setpoint_pub = rospy.Publisher('/mavros/setpoint_raw/local', PositionTarget, queue_size=10)
        '''
        ros services
        '''
    ...
    def start(self):
        ...
        self.get_setpoint_vel(0.2,0,0.5)
        ...
        while not rospy.is_shutdown():
            ...
            if self.local_pose.pose.position.z >2:
                self.get_setpoint_motion(0,0,2,0,0,0,0,0,0,0,0)
            elif self.local_pose.pose.position.z<2:
                self.get_setpoint_motion(0,0,2,0,0,0,0,0,0,0,0)
            # self.get_setpoint_motion(0,0,2,0,0,0,0,0,0,0,0)
            if self.count == 20:
                ...
                print("local_attitude\t roll:%.2f pitch:%.2f yaw:%.2f"%(self.rpy_pose[2],self.rpy_pose[1],self.rpy_pose[0]))
