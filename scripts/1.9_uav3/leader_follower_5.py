...
avoid_vel_x = [0]*(uav_num+1)
avoid_vel_y = [0]*(uav_num+1)
avoid_pos_z = 0.5
avoid_pos_x = 0.1
avoid_pos_y = 0.1
leader_height = 0
...
formation.append( [[-1,-1],[0,-1],[1,-1],[-1,0],[0,0],[1,0]] )  #2x3 formation
                                                              ...
                                                              #  i i i
'''
formation.append( [[0,0],[0,2],[0,4],[0,6],[0,8]]  )  #'I' formation
                                                              #    i
                                                              #    i  
                                                              #    i  
                                                              #    i
formation.append( [[0,0],[2,0],[4,0],[6,0],[8,0]])  #2x3 formation
                                                              #  i i i  i i
                                                              #  
formation.append(  [[0,0],[0,2],[0,-2],[2,0],[4,0]] )#T formation
                                                              #      i i i 
                                                              #        i
                                                              #        i
formation.append(  [[0,0],[0,2],[1.9,-0.6],[1.9,2.6],[3.5,1]] )# formation
                                                              #      i i
                                                              #    i     i
                                                              #       i                                                        
...
def cmd_callback(msg):
    global formation_id
    if not msg.data == '': 
         formation_id = int(msg.data[-1]) #the last element
...
while(1):
        ...
        for j in range(1,uav_num-i):            
            if pow(local_pose[uav_id].pose.position.x-local_pose[uav_id+j].pose.position.x,2)\
                +pow(local_pose[uav_id].pose.position.y-local_pose[uav_id+j].pose.position.y,2)\
                 +pow(local_pose[uav_id].pose.position.z-local_pose[uav_id+j].pose.position.z,2)  < 1.0:
                avoid = True
    ...
    for i in range(uav_num):
        uav_id = i
        if uav_id != leader_id:
            follower_cmd_vel[uav_id].linear.x = leader_cmd_vel.linear.x+delta_vel(formation[formation_id][i][0],relative_pose[uav_id].pose.position.x,KP_xy, max_vel_xy) + avoid_vel_x[uav_id]
            follower_cmd_vel[uav_id].linear.y = leader_cmd_vel.linear.y+delta_vel(formation[formation_id][i][1],relative_pose[uav_id].pose.position.y, KP_xy, max_vel_xy) + avoid_vel_y[uav_id]
            ...
            follower_cmd_vel[uav_id].angular.x = 0.0
            follower_cmd_vel[uav_id].angular.y = 0.0
            follower_cmd_vel[uav_id].angular.z = KP_yaw*(leader_yaw - yaw)
            
            follower_vel_enu_pub[uav_id].publish(follower_cmd_vel[uav_id])

    if hover: 
        #leader_cmd_vel.linear.z = delta_vel(leader_height,local_pose[leader_id].pose.position.z, KP_z, max_vel_z)
        ...
        #leader_cmd_vel.linear.z = leader_cmd_vel.linear.z
