...
class PX4Communication:
    ...
    def __init__(self,id):
        ...
        ros subscribers
        '''    
        self.local_pose_sub = rospy.Subscriber("iris_"+str(self.id)+"/mavros/local_position/pose", PoseStamped, self.local_pose_callback)
        self.mavros_sub = rospy.Subscriber("iris_"+str(self.id)+"/mavros/state", State, self.mavros_state_callback)
        self.imu_sub = rospy.Subscriber("iris_"+str(self.id)+"/mavros/imu/data", Imu, self.imu_callback)
        self.cmd_pose_flu_sub = rospy.Subscriber("/xtdrone/"+"iris_"+str(self.id)+"/cmd_pose_flu", Pose, self.cmd_pose_flu_callback)
        self.cmd_pose_enu_sub = rospy.Subscriber("/xtdrone/"+"iris_"+str(self.id)+"/cmd_pose_enu", Pose, self.cmd_pose_enu_callback)
        self.cmd_vel_flu_sub = rospy.Subscriber("/xtdrone/"+"iris_"+str(self.id)+"/cmd_vel_flu", Twist, self.cmd_vel_flu_callback)
        self.cmd_vel_enu_sub = rospy.Subscriber("/xtdrone/"+"iris_"+str(self.id)+"/cmd_vel_enu", Twist, self.cmd_vel_enu_callback)
        self.cmd_sub = rospy.Subscriber("/xtdrone/"+"iris_"+str(self.id)+"/cmd",String,self.cmd_callback)

        '''
        ros publishers
        '''
        self.pose_target_pub = rospy.Publisher("/iris_"+str(self.id)+"/mavros/setpoint_position/local", PoseStamped, queue_size=10)
        self.twist_target_pub = rospy.Publisher("/iris_"+str(self.id)+"/mavros/setpoint_velocity/cmd_vel", TwistStamped, queue_size=10)

        '''
        ros services
        '''
        self.armService = rospy.ServiceProxy("/iris_"+str(self.id)+"/mavros/cmd/arming", CommandBool)
        self.flightModeService = rospy.ServiceProxy("/iris_"+str(self.id)+"/mavros/set_mode", SetMode)


        print("iris"+str(self.id)+": "+"PX4 Communication Initialized!")
