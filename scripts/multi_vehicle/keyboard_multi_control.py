...
if __name__=="__main__":
    ...
    for i in range(uav_num):
        multi_cmd_vel_flu_pub[i] = rospy.Publisher('/xtdrone/iris_'+str(i)+'/cmd_vel_flu', Twist, queue_size=10)
        multi_cmd_pub[i] = rospy.Publisher('/xtdrone/iris_'+str(i)+'/cmd',String,queue_size=10)
    ...
    try:
        ...
        while(1):
            ...
            if key == 'w' :
                ...
                '''
                if ctrl_leader == True :
                     print('control the leader now')
                else:
                    print('control all drones now')
                '''
            elif key == 'k':
                cmd = 'AUTO.LOITER'
                cmd_vel_mask = False
