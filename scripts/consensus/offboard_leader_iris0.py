...
class Leader:
    def __init__(self):
        ...
        self.local_pose_sub = rospy.Subscriber("/iris_0/mavros/global_position/global", NavSatFix, self.global_pose_callback)
        ...
        while not rospy.is_shutdown():
            ...
            if self.local_pose.pose.position.z >=5.4 and self.local_pose.pose.position.z <= 6:
                if count is None:
                    ...
                    vx = 0.5*w*r*math.sin(w*count*0.05)
                    vy = 0.5*w*r*math.cos(w*count*0.05) 
                    count += 1
                    print(count*0.05)
                    z = 0
            ...
            #     vy = 0
            # print(time.secs)
            self.get_setpoint_motion(0,0,5.5,vx,vy,0,0,0,0,0,0)
            self.motion_setpoint_pub.publish(self.setpoint_motion)
            if self.count == 20:
