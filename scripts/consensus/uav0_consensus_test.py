...
class Leader:
    def __init__(self):
        ...
        self.formationPose_sub =rospy.Subscriber('/iris_2/formation_pose', Pose, self.uav2_formation_pose_callback,queue_size=2)
        self.formationPose_sub =rospy.Subscriber('/iris_2/mavros/local_position/pose', PoseStamped, self.uav2_local_pose_callback,queue_size=2)
        ...
        self.formation_position_pub = rospy.Publisher('/iris_0/formation_pose',Pose,queue_size=2)
        '''
        ros services
        '''
        ...
        while not rospy.is_shutdown():
            ...
            if self.local_pose.pose.position.z >=5.4 and self.local_pose.pose.position.z <= 6:
                    ...
                    if all_ready == 1:
                        ...
                        tmp2_x = self.uav2_local_pose.pose.position.x-self.uav2_formation_pose.position.x
                        tmp2_y = self.uav2_local_pose.pose.position.y-self.uav2_formation_pose.position.y
                        vx = -tmp_x+K*(tmp2_x-tmp_x)
                        vy = -tmp_y+K*(tmp2_y-tmp_y)
    ...
    def uav2_formation_pose_callback(self,msg):
        self.uav2_formation_pose = msg
